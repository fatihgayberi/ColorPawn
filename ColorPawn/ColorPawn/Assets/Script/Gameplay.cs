...
public class Gameplay : MonoBehaviour
{
    ...
    List<RaycastHit> selectedRegionList = new List<RaycastHit>();
    ...
    void TouchAndControl()
    {
        ...
        if (Input.touchCount == 1 && Physics.Raycast(rayArea, out hitArea)) /* Dokunulan objenin ne oldugunu tanimlar */
        {
            if (hitArea.transform.name.Contains("pawn"))
            {
                ...
                selectedRegionSymbol.transform.position = new Vector3(selectedRegionSymbol.transform.position.x, -0.18f, selectedRegionSymbol.transform.position.z + 0.1f); /* yeniden secimleri yapmak isterse secilen bolge ustundeki X'i kaldirir */
            ...
            if (hitArea.transform.name.Contains("region") && RegionEmptyControl(hitArea) && flagSelected) /* secilen objenin region olup olmadigini anlamaya calisir ve ustunde pawn var mi kontrol eder */
            {
                ...
                selectedRegionList.Add(hitArea);
                selectedRegionSymbol.transform.position = new Vector3(hitArea.transform.position.x, 0.07f, hitArea.transform.position.z + 0.1f); /* X'i isaretler */
        ...
        {
            for (int i = 0; i < selectedRegionList.Count; i++)
            {
                if (selectedPawn.collider != null && selectedRegionList[i].collider != null)
                {
                    if (((Mathf.Abs(selectedPawn.transform.position.x - selectedRegionList[i].transform.position.x) == 2f && selectedPawn.transform.position.z == selectedRegionList[i].transform.position.z)
                        || (selectedPawn.transform.position.x == selectedRegionList[i].transform.position.x && Mathf.Abs(selectedPawn.transform.position.z - selectedRegionList[i].transform.position.z) == 2f))
                        && PawnWin(selectedRegionList[i], selectedPawn))
                    {
                        selectedPawn.transform.position = new Vector3(selectedRegionList[i].transform.position.x, selectedPawn.transform.position.y, selectedRegionList[i].transform.position.z);
                        //selectedRegion = new RaycastHit();
                        selectedPawn = new RaycastHit();
